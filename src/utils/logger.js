...
const fullTimestamp = () => new Date().toISOString().replace("T", " ").replace("Z", "");
const shortTime = () => {
    const d = new Date();
    return `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')}`;
};

let auditStream = fs.createWriteStream(path.join(BOT_LOGS_DIR, 'audit.log'), { flags: 'a' });
...
const rotateMcLogIfNeeded = () => {
    ...
    if (!mcStream || isOverTime || isOverLines) {
        ...
        auditLog('info', 'LOG_ROTATE', `Rotated Minecraft log to ${path.basename(currentMcLogFile)}`);
    }
};
